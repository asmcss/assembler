...
type UserFunctionCallback = (...args: string[]) => {[key: string]: string}|string;
const functionRepository: Map<string, UserFunctionCallback> = new Map<string, UserFunctionCallback>();

functionRepository.set('mixin', function (...names: string[]): string {
    return names
...
functionRepository.set('space-x', function (...args: string[]): string {
...
functionRepository.set('stack', function (...args: string[]): string {
...
functionRepository.set('sr-only', function (...args: string[]): string {
    if ((args[0] || 'true') !== 'true') {
